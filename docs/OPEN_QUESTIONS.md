...
聯邦銀行 CSV 樣本已於 2026-07-18 提供並完成 `union-bank-credit-card` 格式解析（真實檔只做本機驗證，repo 僅留去識別 fixture）；其多卡合併帳單的寫入模型另見 Q19。仍需要作者提供國泰、LINE Bank 的**去識別化**帳單各一份（PDF 與/或 CSV）；未有樣本的來源繼續使用 generic-csv / generic-text，不猜銀行格式。
